...
export class JournalComponent implements AfterViewChecked {
  ...
  scrollevent() {
    this.zone.runOutsideAngular(() => {
      ...
      this.scroll_hint_top = this.wasmService.asc.render_scroll_hint(this.scrolling_offset);
      this.prologue_box_top = this.wasmService.asc.render_prologue_box(this.scrolling_offset);


      for (let i = 0; i < this.switch.length; i++) {
        const opacity: number = this.wasmService.asc.render_bg(this.switch[i].getBoundingClientRect().top);
        this.b[i] = opacity;
